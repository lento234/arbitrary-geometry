...
def block2D(n=100):
    '''
    Generates coordinates for a 2D block (circle) with a unit diameter of 1.
    ...
    # In cartesian, with unit length
    #    x = np.concatenate([np.linspace(-R,R,n+1),
    #                        np.repeat(R,n-1),
    #                        np.linspace(R,-R,n+1),
    #                        np.repeat(-R,n)])
    #                        
    #    y = np.concatenate([np.repeat(R,n),
    #                        np.linspace(R,-R,n+1),
    #                        np.repeat(-R,n),
    #                        np.linspace((-R + (2*R/n)), R, n)])
    
    coordinates = np.array([np.concatenate([np.linspace(-R,R,n+1),
                                            np.repeat(R,n-1),
                                            np.linspace(R,-R,n+1),
                                            np.repeat(-R,n)]),

                            np.concatenate([np.repeat(R,n),
                                            np.linspace(R,-R,n+1),
                                            np.repeat(-R,n),
                                            np.linspace((-R + (2*R/n)), R, n)])])
